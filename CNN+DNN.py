...
import numpy as np
import pandas as pdfrom sklearn.svm import SVR
from sklearn.neural_network import MLPRegressor
...
modelx = Sequential()
modelx.add(Dense(64, input_dim=100, activation='tanh'))
modelx.add(Dropout(0))
modelx.add(Dense(64, activation='tanh'))
modelx.add(Dropout(0))
modelx.add(Dense(64, activation='tanh'))
modelx.add(Dropout(0))
modelx.add(Dense(units=1))
modelx.compile(Adagrad(lr=0.0007), loss='mse')
print('train=========')
early_stopping = EarlyStopping(monitor='val_loss', patience=50, verbose=2)
modelx.fit(x_train,y_train,epochs=10000,validation_data=(x_test,y_test),callbacks=early_stopping,batch_size=20)
cost = modelx.evaluate(x_test,y_test)
